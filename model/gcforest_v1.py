# -*- coding: utf-8 -*-
"""
Created on Fri Nov  1 17:47:03 2019
...
#!usr/bin/env python
"""
The code is modified from Pierre-Yves Lablanche's source code of deep forest :
Version : 0.1.6
...
Note:The important_features funciton is added to obtain the permutation feature importances.
...
class gcForest(object):
    ...
    def important_features(self, X, y, X_test, y_test,layer=0):
        '''
        ...
        if y is not None:
            ...
            for irf in range(n_cascadeRF):
                prf.fit(X, y)
                crf.fit(X, y)
                prf_if_list= permutation_importance(prf, X_test, y_test,n_repeats=10,random_state=0,scoring='roc_auc')
                crf_if_list= permutation_importance(crf, X_test, y_test,n_repeats=10,random_state=0,scoring='roc_auc')
                prf_imp_fea=prf_if_list.importances_mean
                crf_imp_fea=crf_if_list.importances_mean
                setattr(self, '_casprf{}_{}'.format(self.n_layer, irf), prf)
                setattr(self, '_cascrf{}_{}'.format(self.n_layer, irf), crf)
                imp_feature.append(prf_imp_fea)
                imp_feature.append(crf_imp_fea)
                prf_crf_pred.append(prf.oob_decision_function_)
                prf_crf_pred.append(crf.oob_decision_function_)
        elif y is None:
            for irf in range(n_cascadeRF):
